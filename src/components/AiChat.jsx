...
function AiChat({ 
  ...
  const [output, setOutput] = useState([
    { input: "", reply: `Cześć ${name || "użytkowniku"}! Jestem tutaj, aby Ci doradzić i pomóc. Możesz zadać mi pytanie lub opisać dokładnie co chcesz wiedzieć, 
    pamiętaj jednak, że nie jestem prawdziwym lekarzem i każdą dolegliwość powinieneś skonsultować ze specjalistą.` }
  ]);
  ...
  const systemPrompt = `
    Jesteś doktorem AI pomagającym analizować dane użytkownika, uświadamiaj jednak użytkownika, że to tylko dobra rada i każdorazowo powinien skonsultować się z prawdziwym lekarzem
        ...
        <div className="h-64 overflow-y-auto border border-gray-700 p-2">
          {output.map((item, index) => (
            <div key={index} className="mb-2">
              {input !== "" && <p className="text-green-400">{"> " + item.input}</p>}
